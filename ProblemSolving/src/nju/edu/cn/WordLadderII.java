...
//	}
	...
	List<List<String>> result;
	Map<String, Set<String>> adjacentGraph;
	List<String> list;
	public List<List<String>> findLadders(String start, String end, Set<String> dict) {
		result = new ArrayList<List<String>>();
		adjacentGraph = new HashMap<String, Set<String>>();
			...
			for (int i = 0; i < string.length(); i++) {
				for (char j = 'a'; j <= 'z'; j++) {
					...
					if (unvisited.contains(newString)) {
						...
						if (adjacentGraph.containsKey(newString)) {
							adjacentGraph.get(newString).add(string);
						} else {
							Set<String> set = new HashSet<String>();
							set.add(string);
							adjacentGraph.put(newString, set);
						}
						
						if (newString.endsWith(end)) {
							found = true;
						}
					}
		...
		backTracing(start,end);
		
		return result;
	}
	
	private void backTracing(String start, String word){
		if (word.equals(start)) {
			list.add(0, start);
			result.add(new ArrayList<String>(list));
			list.remove(0);
		...
		list.add(0, word);
		if (adjacentGraph.get(word) != null) {
			for (String str : adjacentGraph.get(word)) {
				backTracing(start, str);
			}
		}
		list.remove(0);
	}
